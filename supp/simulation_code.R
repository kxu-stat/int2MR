...
source("~/Dropbox/sexbiasedmr/code/data_generation_func_joint_v1UHP.R")
...
model {
  ...
  sigma_gamma_sq  ~ inv_gamma(0.01, 0.01);
  sigma_alpha1_sq ~ inv_gamma(0.01, 0.01);
  sigma_alpha2_sq ~ inv_gamma(0.01, 0.01);
  sigma_alpha3_sq ~ inv_gamma(0.01, 0.01);
...
param_grid <- expand.grid(
  ...
  n3         = c(2000),                         # sample size of single-sex GWAS (female)
  n4         = c(1000),                         # sample size of single-sex GWAS (male)
  true_beta  = c(0.05),                         # main causal effect
  prob1      = c(0.5),                          # proportion male
...
save(
  result_power,
  result_type1error,
  file = "simulation_refactored.RData"
)